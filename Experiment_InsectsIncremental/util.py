...
def run_performance_experiment(stream, dataset, model_specification, detection, prediction_type, targets,
    ...
    model = MCDropoutCLFWrapper(model_specification, targets = targets, epochs=100, mcd_runs=50, debug=False)
...
def get_model(features, targets):
    ...
    x_b = Dense(128, activation='relu')(inputs)
    x_db = Dropout(0.1)(x_b, training=True)
    x_a = Dense(64, activation='relu')(x_db)
