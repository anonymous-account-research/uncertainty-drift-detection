...
def get_model(features, targets):
    ...
    x_d = Dense(512, activation='relu')(inputs)
    x_da = Dropout(0.2)(x_d, training=True)
    x_c = Dense(512, activation='relu')(x_da)
    x_ca = Dropout(0.2)(x_c, training=True)
    x_b = Dense(256, activation='relu')(x_ca)
    x_ba = Dropout(0.1)(x_b, training=True)
    # x_a = Dense(64, activation='relu')(x_ba)
    # x_da = Dropout(0.1)(x_a, training=True)
    # x_0 = Dense(32, activation='relu')(x_da)
    # x_d0 = Dropout(0.1)(x_0, training=True)
    x_1 = Dense(32, activation='relu')(x_ba)
    x_d1 = Dropout(0.1)(x_1, training=True)
    # x_2 = Dense(8, activation='relu')(x_d1)
    # x_d2 = Dropout(0.1)(x_2, training=True)
    outputs = Dense(targets, activation='softmax')(x_d1)
