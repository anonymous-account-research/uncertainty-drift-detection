...
data = arff.loadarff('INSECTS-abrupt_balanced_norm.arff')
data = pd.DataFrame(data[0])
data['class'] = data['class'].astype('category').cat.codes
dataset_name = 'INSECTS-abrupt_balanced_norm'
...
# model_init = MCDropoutCLFWrapper(model, targets=targets, epochs=50, mcd_runs=25, debug=False)
...
#kswin_param = ks_data.gridsearch_kswin_parameter(stream, model_init, 1, targets, features, starting_value=-5)
kswin_alpha = 9e-08
...
#Save data
filename = 'results_' + algorithm + '_' + dataset_name +'2.pickle'
