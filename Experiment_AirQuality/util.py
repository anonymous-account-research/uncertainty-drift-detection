...
def run_performance_experiment(stream, dataset, model_specification, detection, prediction_type, targets,
                               retrain=False, retrain_after=0, refit_use_Xtrain=True, adwin_retrainings=0,
                               adwin_parameter=0.002, features=1):
    dict_metrics = {}
    ...
    model = MCDropoutREGWrapper(model_specification, epochs=200, mcd_runs=100, debug=False)
    ...
    print(mean_squared_error(y_test, y_pred[0]))
...
def get_model(features, targets):
    ...
    x_d1 = Dropout(0.2)(x_1, training=True)  # 0.1
    x_2 = Dense(64, activation='relu')(x_d1)
    x_d2 = Dropout(0.2)(x_2, training=True) #0.1
    x_3 = Dense(32, activation='relu')(x_d2)
    x_d3 = Dropout(0.1)(x_3, training=True)
    x_4 = Dense(16, activation='relu')(x_d3)
    x_d4 = Dropout(0.1)(x_4, training=True)
    outputs = Dense(targets, activation='linear')(x_d4)
    model_reg = Model(inputs, outputs)
    model_reg.compile(optimizer='adam', loss='mse')

    weights = model_reg.get_weights()
    model = [model_reg, weights]
    return model
