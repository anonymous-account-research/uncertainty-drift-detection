...
data = pd.read_csv('air_quality_clean.csv')
dataset_name = 'air'
...
algorithm = 'MCDropout'
#
model_init = MCDropoutREGWrapper(model, debug=False)
# adwin_uncertainty.gridsearch_adwin_parameter(stream, model_init, 1, targets, features)
...
adwin_error.gridsearch_adwin_parameter(stream, model_init, 1, targets, features)
...
#kswin_alpha = ks_data.gridsearch_kswin_parameter(stream, model_init, 1, targets, features)
kswin_alpha = 1e-50
